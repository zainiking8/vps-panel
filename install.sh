...
server {
    ...
    # Dropbear WebSocket over TLS (via wstunnel)
    ...
    # Raw WebSocket SSH endpoint (wstunnel ws://)
    location /ws-ssh {
        if (\$http_upgrade != "websocket") { return 404; }
...
[Service]
Type=simple
ExecStart=/usr/local/bin/wstunnel server wss://0.0.0.0:8881 ws://0.0.0.0:8882 --restrictTo=127.0.0.1:22
Restart=always
RestartSec=3
LimitNOFILE=65535
...
setup_firewall() {
    ...
    for PORT in 22 80 443 109 110 444 8881 10000; do
