...
${G}Usage:${K} sudo bash zainu-vpn.sh [OPTIONS]
  ...
  (no args)        ${Y}auto: install if not installed, else open menu${K}
  --install        full protocol install (re-runs even if already installed)
  --menu           open management menu (after install)
  --check          ${C}audit all services + ports (show what's dead)${K}
  --activate       ${C}start/restart all required services${K}
...
${C}Single-file mode (recommended):${K}
  bash zainu.sh install    --domain vpn.example.com --email me@x.com
  bash zainu.sh check      ${Y}# audit services${K}
  bash zainu.sh activate   ${Y}# restart dead services${K}
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    install|menu|check|activate|status) ACTION="$1"; shift ;;
...
get_inputs(){
  hr
  echo -e "${M}════════ STEP 1: Domain + Certificate Setup ════════${K}"
  echo -e "${Y}Let's Encrypt real TLS certificate banega is domain pe.${K}"
  echo -e "${Y}DNS A record (${PUBIP} → aapka subdomain) pehle set karna padega.${K}"
  hr
  # ALWAYS prompt — saved values become defaults (press Enter to accept)
  local d="${DOMAIN:-}" e="${EMAIL:-}" cf="${CF_HOST:-}" ns="${SLOWDNS_NS:-}"
  if [[ -t 0 ]]; then
    read -r -p "Domain subdomain (A-record → ${PUBIP})${d:+ [$d]}: " v; DOMAIN="${v:-$d}"
    read -r -p "Email for Let's Encrypt cert${e:+ [$e]}: " v; EMAIL="${v:-$e}"
    [[ -z "$EMAIL" ]] && EMAIL="admin@${DOMAIN}"
    read -r -p "Cloudflare host for ws:// links${cf:+ [$cf]} [default=$DOMAIN]: " v; CF_HOST="${v:-$cf:-$DOMAIN}"
    read -r -p "SlowDNS NS subdomain (ns1.your-domain.com)${ns:+ [$ns]}: " v; SLOWDNS_NS="${v:-$ns}"
    read -r -p "SlowDNS password [auto-generate]: " v
    if [[ -z "$v" ]]; then SLOWDNS_PASS="$(rand_str 16)"; wr "SlowDNS password (auto): $SLOWDNS_PASS"; else SLOWDNS_PASS="$v"; fi
  fi
...
install_ssh_dropbear(){
  ...
  # Non-fatal sshd check — try to fix automatically if it fails
  if ! sshd -t 2>>"$LOG"; then
    wr "sshd syntax check failed — auto-fix attempt"
    # Remove duplicate keys that may have been added
    awk '!seen[$1]++ || !/^[A-Z]/{print}' "$S" > "$S.tmp" && mv "$S.tmp" "$S" 2>/dev/null || true
    if ! sshd -t 2>>"$LOG"; then
      wr "sshd still failing — restoring backup"
      [[ -f "$S.bak.$(ls -1t "$S".bak.* 2>/dev/null | head -1 | sed 's/.*\.bak\.//')" ]] && \
        cp "$(ls -1t "$S".bak.* | head -1)" "$S" 2>/dev/null || true
    fi
  fi
...
NGINXEOF
  ...
  # Non-fatal nginx check — auto-recover if config invalid
  if ! nginx -t 2>>"$LOG"; then
    wr "nginx config invalid — removing custom config and using default"
    cat > /etc/nginx/sites-available/default <<NGINXRECOVERYEOF
server {
    listen 80 default_server; listen [::]:80 default_server;
    server_name $DOMAIN _; root /var/www/html; index index.html;
    location / { try_files \$uri \$uri/ =404; }
}
NGINXRECOVERYEOF
    nginx -t 2>>"$LOG" || wr "nginx still failing — will start anyway"
  fi
...
#────────────────────────────── Self-test ────────────────────────────────────
#────────────────────────────── CHECK + ACTIVATE (new) ─────────────────────────
# Use these to find dead services and bring them up. --check audits, --activate restarts dead ones.
ALL_SERVICES=(ssh dropbear stunnel4 nginx xray zainu-slowdns hysteria-server wg-quick@wg0 openvpn@server zainu-ws-ssh zainu-udpgw)
ALL_PORTS=(
  "22/tcp:SSH" "143/tcp:Dropbear" "9443/tcp:SSL-Stunnel" "80/tcp:Nginx-HTTP" "443/tcp:Nginx-TLS"
  "20000/tcp:WS-SSH-bridge" "7100/udp:UDP-Custom" "5300/udp:SlowDNS" "443/udp:Hysteria2"
  "51820/udp:WireGuard" "1194/udp:OpenVPN" "20001/tcp:VMess-WS" "20002/tcp:VLESS-WS"
  "20003/tcp:Trojan-WS" "20004/tcp:VMess-TCP" "8443/tcp:VLESS-Reality" "2053/tcp:Trojan-gRPC" "8388/tcp:Shadowsocks"
)

check_services() {
  hr
  step "🔍 Checking all ZAINU services + ports"
  local fail=0 pass=0 dead_svcs=()
  printf "${M}── SERVICES ──${K}\n"
  for s in "${ALL_SERVICES[@]}"; do
    ...
    if [[ "$st" == "active" ]]; then
      printf "  ${G}✓${K} %-26s ${G}%s${K}\n" "$s" "$st"; pass=$((pass+1))
    else
      printf "  ${R}✗${K} %-26s ${Y}%s${K}\n" "$s" "$st"; fail=$((fail+1)); dead_svcs+=("$s")
    fi
  done
  printf "\n${M}── PORTS ──${K}\n"
  for pd in "${ALL_PORTS[@]}"; do
    ...
    if ss -tuln 2>/dev/null | grep -E ":${port} " >/dev/null; then
      printf "  ${G}✓${K} %-26s ${G}listening${K}\n" "$desc(:$port)"; pass=$((pass+1))
    else
      printf "  ${R}✗${K} %-26s ${Y}down${K}\n" "$desc(:$port)"; fail=$((fail+1))
    fi
  done
  hr
  if [[ $fail -eq 0 ]]; then
    echo -e "${G}✅ All services + ports OK (pass=$pass)${K}"
  else
    echo -e "${Y}⚠️  $fail issue(s) found. Run: ${C}menu${Y} → option 9 (Restart) OR ${C}bash zainu-vpn.sh --activate${K}"
    echo -e "${M}Dead services:${K} ${dead_svcs[*]:-(none)}${K}"
  fi
  hr
}
...
activate_services() {
  hr
  step "🔧 Activating all ZAINU services"
  for s in "${ALL_SERVICES[@]}"; do
    systemctl list-unit-files "${s}.service" 2>/dev/null | grep -q "${s}.service" || { wr "$s — not installed, skipping"; continue; }
    printf "  ${C}→${K} %-26s " "$s"
    if systemctl is-active --quiet "$s"; then
        echo -e "${G}already running${K}"; continue
    fi
    ...
    if systemctl restart "$s" 2>/dev/null; then
        ...
        echo -e "${R}failed${K}"
        # Last-resort: try to start the service fresh
        systemctl daemon-reload 2>/dev/null || true
        systemctl start "$s" 2>/dev/null && echo -e "    ${Y}(started after daemon-reload)${K}" || \
            echo -e "    ${Y}manual fix:  journalctl -u $s -n 30${K}"
    fi
  done
  # Force-start helpers that might not be in systemd
  pgrep -f zainu-udpgw >/dev/null 2>&1 || (nohup /usr/local/bin/zainu-udpgw.py >> "$LOG" 2>&1 &) && wr "udpgw force-started"
  pgrep -f zainu-ws-ssh >/dev/null 2>&1 || (nohup /usr/local/bin/zainu-ws-ssh.py >> "$LOG" 2>&1 &) && wr "ws-ssh bridge force-started"
  hr
  # Re-check
  check_services
}
...
do_install(){
  preflight
  hr; echo -e "${M}════════ INSTALL FLOW START ════════${K}"; hr
  echo -e "${Y}Step 1${K} → ${C}Domain + Certificate${K}     (asks domain, email, SlowDNS)"
  echo -e "${Y}Step 2${K} → ${C}Dependencies${K}             (apt install packages)"
  echo -e "${Y}Step 3${K} → ${C}SSH + Dropbear + Banner${K}  (OpenSSH + Dropbear + brand banner)"
  echo -e "${Y}Step 4${K} → ${C}SSL/TLS + Let's Encrypt${K}  (real cert banega!)"
  echo -e "${Y}Step 5${K} → ${C}WebSocket + Xray + UDP${K}  (all protocols)"
  echo -e "${Y}Step 6${K} → ${C}Hysteria2 + WG + OpenVPN${K} (more protocols)"
  echo -e "${Y}Step 7${K} → ${C}Accounts + Firewall${K}      (system + UFW)"
  echo -e "${Y}Step 8${K} → ${C}Self-test + Links${K}        (PASS/FAIL + links)"
  hr
  get_inputs
  install_deps
  install_brand_banner
...
menu_main(){
  ...
  printf "  ${C}10${K}) ${G}Activate / Fix Dead Services${K} ${Y}(--activate)${K}\n"
  printf "  ${C}11${K}) ${G}System Info${K}\n"
  printf "  ${C}12${K}) ${Y}Re-install / Add Protocols${K}\n"
  printf "  ${C}13${K}) ${R}Uninstall ZAINU${K}\n"
  printf "  ${C} 0${K}) ${R}Exit Panel${K}\n"
  printf "${Y}══════════════════════════════════════════════════════════════════════${K}\n"
  printf "${W}Select [0-13]: ${K}"
  read -r c
}
...
menu(){
  while true; do
    ...
    case "$c" in
      ...
      9)  restart_all; pause ;;
      10) activate_services; pause ;;
      11) sys_info; pause ;;
      12) read -r -p "Re-run full install? Existing config + accounts kept (y/N): " cf
          [[ "$cf" =~ ^[Yy]$ ]] && do_install || wr "cancelled" ;;
      13) read -r -p "Type YES to confirm uninstall: " cf; [[ "$cf" == "YES" ]] && do_uninstall || wr "cancelled" ;;
...
main(){
  ...
  case "$ACTION" in
    ...
    menu)      menu ;;
    auto)
      # If not installed OR config missing → run full install (asks domain → certs → everything)
      # If installed AND config present → open menu
      if is_installed && [[ -s "$ZDIR/config.env" ]] && [[ -f "$MARKER" ]]; then
        menu
      else
        do_install
      fi
