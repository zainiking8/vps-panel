...
do_uninstall(){
  hr; echo -e "${R}Uninstalling $BRAND ... (full clean wipe)${K}"; hr
  ...
  systemctl disable --now \
    ...
    dropbear stunnel4 nginx 2>/dev/null || true
  pkill -f zainu-udpgw 2>/dev/null || true
  pkill -f zainu-ws-ssh 2>/dev/null || true
  pkill -f iodine 2>/dev/null || true
  ok "services stopped"

  # 2) Delete ALL VPN user accounts (from accounts.db) + remove their iptables chains
  step "Deleting VPN user accounts + iptables chains"
  ...
  if [[ -f "$db" ]]; then
    while IFS=: read -r u _ _ _ _ _ _ _; do
      ...
      iptables -D OUTPUT -m owner --uid-owner "$(id -u "$u" 2>/dev/null || echo 0)" -j "ZAINU_${u}" 2>/dev/null || true
    ...
    wr "no accounts.db found"
  fi

  # 3) Flush ALL ZAINU iptables rules globally (catch any leftovers)
  step "Flushing leftover iptables rules"
  iptables -S 2>/dev/null | grep -i zainu | while read -r _ rule; do
  ...
  # 4) Remove config dirs + files
  step "Removing config files + binaries"
  rm -rf /etc/zainu \
         /etc/hysteria \
         /etc/wireguard \
         /etc/openvpn \
         ...
         /var/www/html/zainu* \
         2>/dev/null || true
  ok "configs + binaries removed"

  # 5) Restore default motd/issue/issue.net (remove ZAINU banner)
  step "Restoring default login messages"
  cat > /etc/motd <<'EOF'
...
EOF
  ...
  # Remove zainu-shell from /etc/shells
  [[ -f /etc/shells ]] && sed -i '/zainu-shell/d' /etc/shells 2>/dev/null || true
  ok "login messages restored"

  # 6) Undo sshd_config modifications (remove Banner, restore)
  step "Restoring sshd_config"
  ...
  # 7) Remove cron entries (ALL zainu-related, not just one)
  step "Cleaning cron"
  ( crontab -l 2>/dev/null | grep -v -i zainu ) | crontab - 2>/dev/null || true
  ok "cron cleaned"

  # 8) Remove Let's Encrypt certs
  ...
  apt-get -y remove --purge xray dropbear stunnel4 nginx nginx-common iodine hysteria-server wireguard wireguard-tools openvpn 2>/dev/null || true
  ...
  hr
  echo -e "${G}✅ ${BRAND} fully uninstalled.${K}"
  echo -e "${Y}System is now clean. Reinstall with: ${C}menu --reset${K}"
  hr
  exit 0
}
