...
PYEOF
  ...
  echo "$u:$UID_UUID:$exp:$iplim:$gblim:$uid:xray:$TR_PASS:$SS_PASS:$(date +%s)" >> "$DB"

  # generate links
  local pv="$(printf '%s' "/vless-ws" | sed 's#/#%2F#g')"
  local pt="$(printf '%s' "/trojan-ws" | sed 's#/#%2F#g')"
  local vmess_tls="{\"v\":\"2\",\"ps\":\"${TAG}-${u}-VMess-WS\",\"add\":\"${DOMAIN}\",\"port\":\"443\",\"id\":\"${UID_UUID}\",\"aid\":\"0\",\"net\":\"ws\",\"path\":\"/vmess-ws\",\"tls\":\"tls\",\"sni\":\"${DOMAIN}\"}"
  ...
  local l_hy2="hysteria2://${HY2_PASS:-}@${DOMAIN}:${PORT_HY2}/?sni=${DOMAIN}&insecure=0#${TAG}-${u}-HY2"
