...
> ⚠️ **During install, the script asks for only TWO subdomains — both must have A-records pointing to your VPS IP:**
> - **Panel subdomain** (e.g. `panel.yourdomain.com`) → for 3X-UI panel + SSL
> - **NS subdomain** (e.g. `ns.yourdomain.com`) → for SlowDNS
>
> If the panel subdomain is blank, SSL is skipped (IP-only mode). SSL fails won't block installation.
