...
deploy_cores_and_dns_matrix() {
    ...
    if [[ ! -f /usr/local/bin/xray ]]; then
        bash -c "$(curl -L https://github.comXray-install/raw/main/install-release.sh)" @ install > /dev/null 2>&1
    fi
...
XRAY_JSON_EOF
    ...
    if [[ ! -f /usr/local/bin/dnstt-server ]]; then
        wget -O /usr/local/bin/dnstt-server "https://github.comOnerb12/dnstt-binaries/raw/main/dnstt-server" &>/dev/null
