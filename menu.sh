...
setup_ssl_nginx() {
    ...
    if [[ -z "$subdomain" ]]; then
        msg_warn "No subdomain provided, skipping SSL. Use IP-only setup (Reality best)."
        return
    fi
    ...
    msg_info "Setting up Nginx reverse proxy for ${subdomain}..."
    # HTTP block (acme validation + panel proxy, will be upgraded to HTTPS after cert)
...
EOF
    ...
    if ! nginx -t >/tmp/nginx-test.log 2>&1; then
        ...
        msg_warn "Continuing without SSL..."
        return
    fi
    ...
    if [[ ! -f "$acme" ]]; then
        msg_info "Installing acme.sh (ACME client)..."
        curl -sL https://get.acme.sh | sh -s "email=$email" >/dev/null 2>&1
    fi

    msg_info "Issuing SSL certificate via acme.sh for ${subdomain}..."
    local acme_log="/tmp/acme-zainu.log"
    if "$acme" --issue -d "${subdomain}" --nginx >"$acme_log" 2>&1; then
        # Install cert to fixed, predictable paths
        "$acme" --install-cert -d "${subdomain}" \
            --key-file       "$ssl_dir/privkey.pem" \
            --fullchain-file "$ssl_dir/fullchain.pem" \
            --reloadcmd      "systemctl reload nginx" >/dev/null 2>&1

        # Upgrade nginx to HTTPS
        cat > /etc/nginx/sites-available/zainu-x-brand <<EOF
server {
    listen 80;
    listen [::]:80;
...
EOF
        ...
        msg_ok "SSL installed via acme.sh"
        echo "SSL Cert: $ssl_dir/fullchain.pem" >> "$CREDENTIALS"
        echo "SSL Key : $ssl_dir/privkey.pem" >> "$CREDENTIALS"
        echo "Panel   : https://${subdomain}" >> "$CREDENTIALS"
    else
        msg_err "acme.sh SSL failed. Details:"
        cat "$acme_log"
        echo ""
        msg_warn "Fixes:"
        msg_warn "  1. Ensure '${subdomain}' A-record → this VPS IP (check: dig +short ${subdomain})"
        msg_warn "  2. Wait 5-30 min after DNS change, then re-run: ~/.acme.sh/acme.sh --issue -d ${subdomain} --nginx"
        msg_warn "  3. Port 80 must be open & free."
        msg_warn "Continuing without SSL. Panel: http://$(curl -s ifconfig.me):54321"
    fi
    # acme.sh installs its own auto-renew cron automatically
}
...
install_slowdns() {
    ...
    # NS subdomain provided by user (points to VPS IP)
    NS_DOMAIN=${NS_DOMAIN:-ns}
...
run_installation() {
    ...
    # Inputs — only ONE panel subdomain + ONE NS subdomain (both point to VPS IP)
    echo -e "${CYAN}Enter panel subdomain (A-record → VPS IP, e.g. panel.yourdomain.com) — blank for IP-only:${NC}"
    read -r PANEL_DOMAIN
    email="admin@${PANEL_DOMAIN:-localhost}"
    if [[ -n "$PANEL_DOMAIN" ]]; then
        echo -e "${CYAN}Enter email for SSL certificate:${NC}"
        read -r email
    fi

    echo -e "${CYAN}Enter SlowDNS NS subdomain (A-record → VPS IP, e.g. ns.yourdomain.com) [ns]:${NC}"
    read -r NS_DOMAIN
    NS_DOMAIN=${NS_DOMAIN:-ns}

    mkdir -p "$INSTALL_DIR" "$CONFIG_DIR"
    echo "Installation Date: $(date)" > "$CREDENTIALS"
    echo "Panel Subdomain: ${PANEL_DOMAIN:-IP-Only}" >> "$CREDENTIALS"
    echo "SlowDNS NS: ${NS_DOMAIN}" >> "$CREDENTIALS"
    ...
    setup_firewall
    install_3x_ui
    setup_ssl_nginx "$PANEL_DOMAIN" "$email"
    install_slowdns
    install_udp_custom
    setup_ssh_limits
    ...
    cat "$CREDENTIALS"
    echo ""
    echo -e "${YELLOW}${BOLD}📡 Next step to connect VPN:${NC}"
    echo -e " 1) Open panel: ${MAGENTA}https://${PANEL_DOMAIN:-$(curl -s ifconfig.me):54321}${NC}"
    echo -e " 2) Login with 3X-UI creds above → Inbounds → Add Inbound"
    echo -e " 3) VLESS+Reality (IP, no SSL) OR VLESS+WS+TLS (use cert paths above)"
    echo -e " 4) Copy client link → import in v2rayNG/Shadowrocket → Connect 👑"
    echo ""
    echo -e "${MAGENTA}Run '${BOLD}menu${NC}${MAGENTA}' → option 8 for SSL & Panel info.${NC}"
    pause
}
...
run_uninstall() {
    ...
    # SSL certificates
    msg_info "Removing SSL certificates (acme.sh + panel ssl)..."
    if [[ -f "$HOME/.acme.sh/acme.sh" ]]; then
        "$HOME/.acme.sh/acme.sh" --remove -d "${PANEL_DOMAIN:-none}" >/dev/null 2>&1
    fi
    rm -rf "$HOME/.acme.sh" >/dev/null 2>&1
    rm -rf "$INSTALL_DIR/ssl" >/dev/null 2>&1
...
show_ssl_info() {
    ...
    echo -e "${CYAN}${BOLD}SSL & Panel Information${NC}"
    echo ""
    echo -e "${YELLOW}Use these paths in 3X-UI inbound TLS settings:${NC}"
    echo ""
    if [[ -f "$ssl_dir/fullchain.pem" ]]; then
        echo -e " ${WHITE}Certificate :${NC} ${GREEN}$ssl_dir/fullchain.pem${NC}"
        echo -e " ${WHITE}Private Key  :${NC} ${GREEN}$ssl_dir/privkey.pem${NC}"
    else
        echo -e " ${RED}SSL not installed.${NC} Run SSL setup or use IP-only (Reality)."
    fi
    echo ""
    echo -e " ${WHITE}Panel Subdomain :${NC} ${PANEL_DOMAIN:-(not set)}"
    echo -e " ${WHITE}Panel URL (SSL) :${NC} https://${PANEL_DOMAIN:-$ip}"
    echo -e " ${WHITE}Panel URL (IP)  :${NC} http://${ip}:54321"
    echo -e " ${WHITE}SlowDNS NS      :${NC} ${NS_DOMAIN:-(not set)}"
    echo ""
    echo -e "${YELLOW}${BOLD}How to make VPN connect:${NC}"
    echo -e " 1) Open panel URL above, login (creds in $CREDENTIALS)"
    echo -e " 2) Inbounds → Add Inbound → VLESS/VMess/Trojan"
    echo -e " 3) For TLS: set cert path = $ssl_dir/fullchain.pem, key = $ssl_dir/privkey.pem"
    echo -e " 4) For Reality (IP, no SSL needed): choose VLESS + Reality + XTLS-Vision"
    echo -e " 5) Copy client link → import in v2rayNG/Shadowrocket → Connect ✅"
    echo ""
    pause
}
...
main_menu() {
    while true; do
        ...
        echo -e "${CYAN} 8)${NC} SSL & Panel Info"
        echo -e "${CYAN} 9)${NC} System Info"
        echo -e "${CYAN}10)${NC} Reboot Server"
        echo -e "${RED}11)${NC} Uninstall Panel (Fresh Clean)"
        ...
        case "$opt" in
            ...
            8) show_ssl_info ;;
            9) show_system_info ;;
            10) read -p "Reboot now? [y/N]: " r; [[ "$r" =~ ^[Yy]$ ]] && reboot ;;
            11) run_uninstall ;;
