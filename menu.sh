...
show_payload() {
    ...
    UUID=$(get_uuid)
    echo -e "${Y}=====================================================${N}"
    echo -e "${G}        [9] CONNECTION INFO & PAYLOADS${N}"
    echo -e "${Y}=====================================================${N}"
    echo ""
    echo -e "${W}Host / Domain : ${C}$DOMAIN${N}"
    echo -e "${W}Server IP     : ${C}$IP${N}"
    echo -e "${W}VLESS UUID    : ${C}$UUID${N}"
    echo ""
    echo -e "${Y}--- VLESS WS TLS (Port 443) ---${N}"
    echo -e "${C}vless://${UUID}@${DOMAIN}:443?type=ws&security=tls&path=/vless-ws&sni=${DOMAIN}#ZAINIX-VLESS${N}"
    echo ""
    echo -e "${Y}--- VLESS WS non-TLS (Port 80) ---${N}"
    echo -e "${C}vless://${UUID}@${DOMAIN}:80?type=ws&security=none&path=/vless-ws#ZAINIX-VLESS${N}"
    echo ""
    echo -e "${Y}--- SSH WebSocket (DarkTunnel style) ---${N}"
    echo -e "${C}Target: ${DOMAIN}:443@username:password${N}"
    echo -e "${C}Proxy : applynow.hdfc.bank.in:443${N}"
    echo -e "${C}SNI   : ${DOMAIN}${N}"
    echo ""
    echo -e "${C}Payload:${N}"
    echo -e "${W}GET / HTTP/1.1[crlf]Host: ${DOMAIN}[crlf]Upgrade: websocket[crlf]Connection: <div><span style=\"color: #0000ff\"> ✨❤@ZAINU X BRAND ➡| 03077716993🇵🇰| ✈ •🪶(BOY) 🇸 🅦</span></div> Upgrade[crlf]User-Agent: [ua][crlf][crlf]${N}"
    echo ""
    echo -e "${Y}--- Server Banner ---${N}"
    echo -e "${G}=====================================================${N}"
