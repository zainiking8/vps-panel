...
if [[ -z "$PASS" ]]; then echo -e "${R}Password cannot be empty!${N}"; sleep 1; return; fi
    ...
    echo -e "${C}GET /ws-ssh HTTP/1.1[crlf]Host: $DOMAIN[crlf]Upgrade: websocket[crlf]Connection: Upgrade[crlf]User-Agent: [ua][crlf][crlf]${N}"
    echo ""
    echo -e "${G}=====================================================${N}"
    echo ""
...
show_payload() {
    ...
    echo ""
    echo -e "${C}Payload (Sahi Working WebSocket Protocol):${N}"
    echo -e "${W}GET /ws-ssh HTTP/1.1[crlf]Host: ${DOMAIN}[crlf]Upgrade: websocket[crlf]Connection: Upgrade[crlf]User-Agent: [ua][crlf][crlf]${N}"
    echo ""
    read -p "Press Enter to continue..."
}
