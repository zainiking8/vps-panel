#!/bin/bash
# ==============================================================================
# PART 1: METROPOLIS NEON SYSTEM INITIALIZATION & THEME COLORS
# ==============================================================================
...
clear
# ==============================================================================
# PART 2: PACKAGE MANAGER REFLECTION & UTILITIES COMPILER
# ==============================================================================
...
# ==============================================================================
# PART 3: HARDWARE ACCELERATION & TCP FLOW OPTIMIZATION
# ==============================================================================
optimize_network_bbr() {
    ...
    echo -e "${MATRIX_GREEN}[✓] Data flow latency boosters successfully locked!${NC}"
}
# ==============================================================================
# PART 4: ENCRYPTION ENVIRONMENT & PREMIUM ACCOUNT TERMINAL BANNER
# ==============================================================================
configure_dropbear_and_banner() {
    echo -e "${NEON_CYAN}[*] Patching host encryption wrappers to circumvent dropbear errors...${NC}"
...
# ==============================================================================
# PART 5: MULTI-THREADED DATA INGESTION ANALYSIS & USER MONITOR DAEMON
# ==============================================================================
            ...
            with open(io_file, "r") as f:
                for line in f:
                    if line.startswith("rchar:") or line.startswith("wchar:"):
                        total_bytes += int(line.split(":").strip())
        except Exception: pass
    return total_bytes
last_pid_bytes = {}
while True:
            ...
            for fname in os.listdir(USER_DIR):
                ...
                for line in lines:
                    if line.startswith("IP_LIMIT="):
                        try: ip_limit = int(line.strip().split("="))
                        except Exception: pass
                    elif line.startswith("GB_LIMIT="): gb_limit = line.strip().split("=")
                    elif line.startswith("USED_MB="):
                        try: used_mb = float(line.strip().split("="))
                        except Exception: pass
...
SVC_EOF
    ...
    echo -e "${MATRIX_GREEN}[✓] Security monitoring active in background threads!${NC}"
}
# ==============================================================================
# PART 6: MULTI-PROTOCOL TUNNEL ENGINE PIPELINES
# ==============================================================================
deploy_cores_and_dns_matrix() {
    echo -e "${CYBER_PINK}[*] Building unified clean-object tracking engine layouts...${NC}"
    ...
    cat << 'XRAY_JSON_EOF' > /etc/xray/config.json
{
  ...
  "inbounds": [
    { "port": 1443, "protocol": "vless", "settings": { "clients": [], "decryption": "none" }, "streamSettings": { "network": "ws", "wsSettings": { "path": "/zainuxbrand-vless" } } },
    { "port": 8081, "protocol": "vless", "settings": { "clients": [], "decryption": "none" }, "streamSettings": { "network": "ws", "wsSettings": { "path": "/zainuxbrand-vless-nt" } } },
    { "port": 1553, "protocol": "trojan", "settings": { "clients": [] }, "streamSettings": { "network": "ws", "wsSettings": { "path": "/zainuxbrand-trojan" } } }
  ],
...
XRAY_JSON_EOF
    ...
    echo -e "${MATRIX_GREEN}[✓] Multi-protocol listening schemas and security keys generated!${NC}"
}
# ==============================================================================
# PART 7: DOMAIN OVERRIDE CONTROLLER & PRODUCTION ENGINE ROUTING
# ==============================================================================
...
NGX_ADV_EOF
    ...
    ufw allow 80/tcp; ufw allow 443/tcp; ufw allow 22/tcp; ufw allow 109/tcp; ufw allow 447/tcp; ufw allow 2082/tcp; ufw allow 53/udp
    echo "y" | ufw enable > /dev/null 2>&1
    echo -e "${MATRIX_GREEN}[✓] High speed proxy traffic pathways active!${NC}"
    press_any_key
}
# ==============================================================================
# PART 8: MULTI-THREADED SOCKET TRANSLATOR CORE PIPELINE
# ==============================================================================
...
SVC_EOF
    ...
    echo -e "${MATRIX_GREEN}[✓] Data transmission broker layer launched successfully!${NC}"
}
# ==============================================================================
# PART 9: TELEGRAM REMOTE SIGNALS CONTROLLER PIPELINES
# ==============================================================================
...
# ==============================================================================
# PART 10: DYNAMIC USER REGISTRY PANELS & LINK GENERATOR MATRIX Blueprints
# ==============================================================================
check_connected_ips() {
    clear
    echo -e "${NEON_CYAN}┏──────────────────────────────────────────────────┓${NC}"
...
advanced_protocols_menu() {
    while true; do
        ...
        case $adv_opt in
            1) read -rp " Username: " vname; local uuid=$(uuidgen); jq ".inbounds[0].settings.clients += [{\"id\": \"$uuid\", \"email\": \"$vname\"}]" /etc/xray/config.json > /tmp/xray.json && mv /tmp/xray.json /etc/xray/config.json; systemctl restart xray
               echo -e "\n${MATRIX_GREEN}====== VLESS TLS CONFIG DETAILS ======${NC}"
               echo -e "UUID      : ${WHITE}${uuid}${NC}"
               echo -e "Import Link:\n${CYAN}vless://${uuid}@${cur_dom}:443?path=%2Fzainuxbrand-vless&security=tls&encryption=none&type=ws#${vname}${NC}"; press_any_key ;;
            2) read -rp " Username: " vname; local uuid=$(uuidgen); jq ".inbounds[1].settings.clients += [{\"id\": \"$uuid\", \"email\": \"$vname\"}]" /etc/xray/config.json > /tmp/xray.json && mv /tmp/xray.json /etc/xray/config.json; systemctl restart xray
               echo -e "\n${MATRIX_GREEN}====== VLESS NON-TLS CONFIG DETAILS ======${NC}"
               echo -e "UUID      : ${WHITE}${uuid}${NC}"
               echo -e "Import Link:\n${CYAN}vless://${uuid}@${cur_dom}:80?path=%2Fzainuxbrand-vless-nt&security=none&encryption=none&type=ws#${vname}${NC}"; press_any_key ;;
            3) read -rp " Passphrase: " tpass; jq ".inbounds[2].settings.clients += [{\"password\": \"$tpass\", \"email\": \"$tpass\"}]" /etc/xray/config.json > /tmp/xray.json && mv /tmp/xray.json /etc/xray/config.json; systemctl restart xray
               echo -e "\n${MATRIX_GREEN}====== TROJAN TLS CONFIG DETAILS ======${NC}"
               echo -e "Import Link:\n${CYAN}trojan://${tpass}@${cur_dom}:443?path=%2Fzainuxbrand-trojan&security=tls&type=ws#${tpass}${NC}"; press_any_key ;;
            4) clear; echo -e "${YELLOW}====== SLOWDNS INFRASTRUCTURE PROFILE ======${NC}"
               if [[ -f /etc/slowdns/server.pub ]]; then echo -e "Public Key  : ${MATRIX_GREEN}$(cat /etc/slowdns/server.pub)${NC}\nPayload WS  : ${cur_dom}"; else echo -e "SlowDNS inactive."; fi; press_any_key ;;
            0|*) return ;;
        esac
    done
}
    ...
    while true; do
        ...
        case $u_choice in
            1) read -rp " Username: " username; read -rp " Password: " password; read -rp " Expiry Days: " days; read -rp " IP Limit: " ip_limit; read -rp " Quota GB: " gb_limit
               ...
               echo -e "Username   : ${WHITE}${username}${NC}\nPassword   : ${WHITE}${password}${NC}"
               echo -e "Payload Custom Header:\n${CYAN}GET / HTTP/1.1[crlf]Host: ${cur_dom}[crlf]Upgrade: websocket[crlf][crlf]${NC}"
               press_any_key ;;
...
uninstall_panel() {
    clear
    echo -e "${ALERT_RED}⚠️ WARNING: YOU ARE DELETING THE ENTIRE INSTANCE PANEL ENGINE ⚠️${NC}"
    read -p "Are you absolutely sure? (y/n): " un_confirm
    if [ "$un_confirm" == "y" ]; then
        systemctl stop nginx dropbear xray autokill ws-proxy tgbot >/dev/null 2>&1
        systemctl disable nginx dropbear xray autokill ws-proxy tgbot >/dev/null 2>&1
        rm -rf /etc/nginx/conf.d/vpn.conf /etc/xray /etc/slowdns /etc/zainuxbrand /usr/local/bin/autokill.py /usr/local/bin/ws-proxy.py /etc/systemd/system/autokill.service /etc/systemd/system/ws-proxy.service /etc/systemd/system/tgbot.service
        systemctl daemon-reload
        echo -e "${MATRIX_GREEN}[✓] Panel structures completely uninstalled from system directories.${NC}"
    fi
    press_any_key
}
