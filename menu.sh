#!/bin/bash
# ==============================================================================
# PART 1: NEON CORE INITIALIZATION & CYBERPUNK STYLING VARIABLES
# ==============================================================================
export LANG=en_US.UTF-8

# Application Identity & System Storage Tree Mappings
...
# Premium Neon Color Configurations
NC='\033[0m'
WHITE='\033[1;37m'
...
# Secure Root Level Context Validation
if [[ $EUID -ne 0 ]]; then
   echo -e "${ALERT_RED}[FATAL ERROR] System root authorization missing! Execute: sudo -i${NC}"
   exit 1
fi
...
press_any_key() {
    echo -e "\n${GOLD_YELLOW}➔ Press [ENTER] key to return to mainframe...${NC}"
    read -r
}

clear
# ==============================================================================
# PART 2: CORE BINARY LIBRARIES & ENVIRONMENT UTILITIES DEPLOYER
# ==============================================================================
install_system_packages() {
    ...
    echo -e "${MATRIX_GREEN}[✓] System production environment compiled safely!${NC}"
    sleep 1
}
# ==============================================================================
# PART 3: TCP GOOGLE BBR CONGESTION CONTROL SPEED ACCELERATOR
# ==============================================================================
optimize_network_bbr() {
    echo -e "${CYBER_PINK}[*] Tuning network interfaces via Google BBR optimization...${NC}"
    
    # Cleaning older kernel network buffer constraints
    sed -i '/net.core.default_qdisc/d' /etc/sysctl.conf
    sed -i '/net.ipv4.tcp_congestion_control/d' /etc/sysctl.conf
    
    # Injecting high-speed throughput network parameters
    echo "net.core.default_qdisc=fq" >> /etc/sysctl.conf
    echo "net.ipv4.tcp_congestion_control=bbr" >> /etc/sysctl.conf
    
    sysctl -p > /dev/null 2>&1
    echo -e "${MATRIX_GREEN}[✓] High-throughput data stream boosters locked into kernel!${NC}"
}
# ==============================================================================
# PART 4: CRYPTO HANDSHAKE PATCH & PRESTIGE INTERFACE CONNECTION BANNER
# ==============================================================================
configure_dropbear_and_banner() {
    echo -e "${NEON_CYAN}[*] Patching host encryption wrappers to prevent dropbear exit errors...${NC}"
    mkdir -p /etc/dropbear && chmod 700 /etc/dropbear
    rm -f /etc/dropbear/dropbear_*_host_key
    
    # Generate balanced secure algorithm keys to circumvent runtime terminations
    ...
    # Formatting custom rebranded premium user notification block
...
BANNER_EOF
    ...
    # Allocating explicit multi-ports to Dropbear service daemon environment
...
DB_CONF
    ...
    echo -e "${MATRIX_GREEN}[✓] Cyber Dropbear Welcome Banner and Ports Operational!${NC}"
}
# ==============================================================================
# PART 5: ADVANCED BANDWIDTH TRACKER & DYNAMIC USER LOCK MECHANISM
# ==============================================================================
...
PY_EOF
    ...
    # Constructing isolated Systemd Unit File Hook
...
SVC_EOF
    ...
    echo -e "${MATRIX_GREEN}[✓] Data quota live monitoring enforcement engine ready!${NC}"
}
# ==============================================================================
# PART 6: MULTI-PROTOCOL TUNNEL CORES MAPPING (XRAY ENGINE & SLOWDNS)
# ==============================================================================
deploy_cores_and_dns_matrix() {
    echo -e "${CYBER_PINK}[*] Constructing core structural proxy inbound schemas...${NC}"
    ...
    cat << 'XRAY_JSON_EOF' > /etc/xray/config.json
{
  ...
  "inbounds": [
    { "port": 1443, "protocol": "vless", "settings": { "clients": [], "decryption": "none" }, "streamSettings": { "network": "ws", "wsSettings": { "path": "/zainuxbrand-vless" } }, "tag": "vless-tls" },
    { "port": 8081, "protocol": "vless", "settings": { "clients": [], "decryption": "none" }, "streamSettings": { "network": "ws", "wsSettings": { "path": "/zainuxbrand-vless-nt" } }, "tag": "vless-nontls" },
    { "port": 1553, "protocol": "trojan", "settings": { "clients": [] }, "streamSettings": { "network": "ws", "wsSettings": { "path": "/zainuxbrand-trojan" } }, "tag": "trojan-tls" }
  ],
...
XRAY_JSON_EOF
    ...
    # Setting up SlowDNS standalone components mapping layers
    mkdir -p /etc/slowdns
    ...
    echo -e "${MATRIX_GREEN}[✓] Xray Engine & SlowDNS configuration arrays active!${NC}"
}
# ==============================================================================
# PART 7: ZERO-API MANUAL SUBDOMAIN INJECTOR & NGINX ACCELERATION CHANNELS
# ==============================================================================
create_subdomain_and_nginx() {
    ...
    echo -e "${NEON_CYAN}┗──────────────────────────────────────────────────┛${NC}"
    echo -e "${GOLD_YELLOW}[➔] Paste your pre-pointed subdomain host directly (No API Key Required):${NC}"
    echo -e "Example format: zainu.uvlist.filegear-sg.me"
    echo -e "${SHADOW_GREY}────────────────────────────────────────────────────${NC}"
    read -p " Subdomain Node: " SUB_FULL

    if [[ -z "$SUB_FULL" ]]; then 
        SUB_FULL=$(curl -s ifconfig.me)
    fi

    mkdir -p /etc/zainuxbrand && echo "$SUB_FULL" > "$DOMAIN_FILE"
    echo -e "${MATRIX_GREEN}[✓] Server targeting environment variable bound to: $SUB_FULL${NC}"

    echo -e "${CYBER_PINK}[*] Issuing automated Let's Encrypt SSL layers via HTTP challenges...${NC}"
    ...
    # Constructing advanced isolated location reverse forward structures
    cat << NGX_ADV_EOF > /etc/nginx/conf.d/vpn.conf
server {
    listen 80; server_name ${SUB_FULL} _;
    ...
    location /zainuxbrand-vless-nt { proxy_pass http://127.0.0.1:8081; proxy_http_version 1.1; proxy_set_header Upgrade \$http_upgrade; proxy_set_header Connection "upgrade"; proxy_set_header Host \$host; }
}
server {
    listen 443 ssl http2; server_name ${SUB_FULL} _;
    ssl_certificate /etc/letsencrypt/live/${SUB_FULL}/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/${SUB_FULL}/privkey.pem;
    location / { proxy_pass http://127.0.0.1:2082; proxy_http_version 1.1; proxy_set_header Upgrade \$http_upgrade; proxy_set_header Connection "upgrade"; proxy_set_header Host \$host; }
    location /zainuxbrand-vless { proxy_pass http://127.0.0.1:1443; proxy_http_version 1.1; proxy_set_header Upgrade \$http_upgrade; proxy_set_header Connection "upgrade"; proxy_set_header Host \$host; }
...
NGX_ADV_EOF
    ...
    # Committing firewall protection layer permissions
    ufw allow 80/tcp; ufw allow 443/tcp; ufw allow 22/tcp; ufw allow 109/tcp; ufw allow 447/tcp; ufw allow 2082/tcp
    echo "y" | ufw enable > /dev/null 2>&1
    echo -e "${MATRIX_GREEN}[✓] High-speed Nginx proxy tunnels active!${NC}"
    press_any_key
}
# ==============================================================================
# PART 8: MULTI-THREADED INDUSTRIAL PYTHON WEBSOCKET SOCKET ENGINE
# ==============================================================================
deploy_python_websocket_proxy() {
    echo -e "${YELLOW}[*] Spawning multi-thread Python WebSocket router on local port 2082...${NC}"
...
SVC_EOF
    ...
    echo -e "${MATRIX_GREEN}[✓] Core Python WebSocket Proxy Active!${NC}"
}
# ==============================================================================
# PART 9: TELEGRAM CORE INTERFACE CONFIGURATOR MICROSERVICE
# ==============================================================================
...
SVC_EOF
    ...
    echo -e "${MATRIX_GREEN}[✓] Telegram microservice automation loop active!${NC}"
    press_any_key
}
# ==============================================================================
# PART 10: ADVANCED MANAGEMENT SUB-MENUS & CYBER MASTER MAIN PANEL LOOP
# ==============================================================================
check_connected_ips() {
    clear
    echo -e "${NEON_CYAN}┏──────────────────────────────────────────────────┓${NC}"
    ...
    echo -e " Total Live Active Network Sockets: ${WHITE}${total_count}${NC}"
    press_any_key
}
...
advanced_protocols_menu() {
    while true; do
        ...
        echo -e "${CYBER_PINK}────────────────────────────────────────────────────${NC}"
        echo -e " ${NEON_CYAN}[1]${NC} Compile VLESS WebSocket Secure (TLS)"
        echo -e " ${NEON_CYAN}[2]${NC} Compile VLESS WebSocket Unsecured (Non-TLS)"
        echo -e " ${NEON_CYAN}[3]${NC} Compile Trojan WebSocket Secure (TLS)"
        echo -e " ${NEON_CYAN}[4]${NC} Query Active SlowDNS Public Crypt-Keys"
        echo -e " ${ALERT_RED}[0]${NC} Return to Control Core"
        echo -e "${CYBER_PINK}────────────────────────────────────────────────────${NC}"
        read -rp " Select Node Action: " adv_opt
        case $adv_opt in
            1) read -rp " Username: " vname; local uuid=$(uuidgen); jq ".inbounds.settings.clients += [{\"id\": \"$uuid\", \"email\": \"$vname\"}]" /etc/xray/config.json > /tmp/xray.json && mv /tmp/xray.json /etc/xray/config.json; systemctl restart xray
               echo -e "\n${MATRIX_GREEN}Generated link below:${NC}\nvless://${uuid}@${cur_dom}:443?path=%2Fzainuxbrand-vless&security=tls&encryption=none&type=ws#${vname}"; press_any_key ;;
            2) read -rp " Username: " vname; local uuid=$(uuidgen); jq ".inbounds.settings.clients += [{\"id\": \"$uuid\", \"email\": \"$vname\"}]" /etc/xray/config.json > /tmp/xray.json && mv /tmp/xray.json /etc/xray/config.json; systemctl restart xray
               echo -e "\n${MATRIX_GREEN}Generated link below:${NC}\nvless://${uuid}@${cur_dom}:80?path=%2Fzainuxbrand-vless-nt&security=none&encryption=none&type=ws#${vname}"; press_any_key ;;
            3) read -rp " Passphrase: " tpass; jq ".inbounds.settings.clients += [{\"password\": \"$tpass\", \"email\": \"$tpass\"}]" /etc/xray/config.json > /tmp/xray.json && mv /tmp/xray.json /etc/xray/config.json; systemctl restart xray
               echo -e "\n${MATRIX_GREEN}Generated link below:${NC}\ntrojan://${tpass}@${cur_dom}:443?path=%2Fzainuxbrand-trojan&security=tls&type=ws#${tpass}"; press_any_key ;;
            4) clear; if [[ -f /etc/slowdns/server.pub ]]; then echo -e "Public Key: ${MATRIX_GREEN}$(cat /etc/slowdns/server.pub)${NC}"; else echo -e "SlowDNS inactive."; fi; press_any_key ;;
            0|*) return ;;
        esac
    done
}
...
user_menu() {
    while true; do
        ...
        echo -e "${MATRIX_GREEN}⚡──────────────────────────────────────────────────⚡${NC}"
        echo -e " ${NEON_CYAN}[1]${NC} Forge New Account (SSH / WebSocket Layer)"
        echo -e " ${NEON_CYAN}[2]${NC} Destroy Target Active User Node"
        echo -e " ${NEON_CYAN}[3]${NC} Diagnostic Check Connected IPs"
        echo -e " ${NEON_CYAN}[4]${NC} Check User Status, Quota, Used Data & Limits"
        echo -e " ${ALERT_RED}[0]${NC} Return to Control Core"
        echo -e "${MATRIX_GREEN}────────────────────────────────────────────────────${NC}"
        ...
        case $u_choice in
            1) read -rp " Username: " username; read -rp " Password: " password; read -rp " Expiry Days: " days; read -rp " IP Limit: " ip_limit; read -rp " Quota GB: " gb_limit
               ...
               echo -e "${MATRIX_GREEN}User account $username active!${NC}"; press_any_key ;;
...
master_deployment_trigger() {
    ...
    deploy_cores_and_dns_matrix
    deploy_python_websocket_proxy
    echo -e "\n${MATRIX_GREEN}[SUCCESS SYSTEM UPDATE] New Cyber Panel Framework Installed Flawlessly!${NC}"
    press_any_key
}
...
master_panel_loop() {
    while true; do
        ...
        echo -e "${NEON_CYAN}🛠️ SYSTEM MAINFRAME : ${WHITE}${PANEL_NAME}${NC}"
        echo -e "${NEON_CYAN}🛰️ DOMAIN INSTANCE  : ${GOLD_YELLOW}${CURRENT_DOM}${NC}"
        echo -e "${NEON_CYAN}📂 CORE DIRECTORY   : ${WHITE}${CUSTOM_PATH}${NC}"
        echo -e "${NEON_CYAN}🌲──────────────────────────────────────────────────🌲${NC}"
        echo -e " ${CYBER_PINK}[1]${NC} Auto Deploy New High-End Cyber Components Layout"
        echo -e " ${CYBER_PINK}[2]${NC} Setup Manual Pre-Pointed / Free Subdomain Override"
        echo -e " ${CYBER_PINK}[3]${NC} Account Control Core (Add, Delete & Active Quota)"
        echo -e " ${CYBER_PINK}[4]${NC} Advanced Tunneling Matrix (VLESS, Trojan & SlowDNS)"
        echo -e " ${CYBER_PINK}[5]${NC} Bind Management Bot to Telegram Hooks API"
        echo -e " ${CYBER_PINK}[6]${NC} Open Live Hardware Resource Diagnostics (htop)"
        echo -e " ${ALERT_RED}[0]${NC} Terminate Current Shell Session Context"
        echo -e "${NEON_CYAN}🌲──────────────────────────────────────────────────🌲${NC}"
        read -rp " Select Mainframe Action Sequence [0-6]: " opt
        case $opt in
...
# Core Interface Sequence Trigger Call
master_panel_loop
