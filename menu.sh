...
deploy_cores_and_dns_matrix() {
    ...
    # Fixed explicit tags object properties schema structure mapping
    mkdir -p /etc/xray
...
# ==============================================================================
# PART 10: USER REGISTRY PANELS, FIXED EXPLICIT TAG MODIFIERS & TOTAL CLEAN PURGE
# ==============================================================================
check_connected_ips() {
    clear
    echo -e "${NEON_CYAN}┏──────────────────────────────────────────────────┓${NC}"
...
advanced_protocols_menu() {
    while true; do
        ...
        case $adv_opt in
            ...
            4) clear; echo -e "${YELLOW}====== SLOWDNS INFRASTRUCTURE PROFILE ======${NC}"; if [[ -f /etc/slowdns/server.pub ]]; then echo -e "Public Key  : ${MATRIX_GREEN}$(cat /etc/slowdns/server.pub)${NC}\nPayload WS  : ${cur_dom}"; else echo -e "SlowDNS inactive."; fi; press_any_key ;;
            0|*) return ;;
        esac
    done
}
    ...
    while true; do
        ...
        case $u_choice in
            1) read -rp " Username: " username; read -rp " Password: " password; read -rp " Expiry Days: " days; read -rp " IP Limit: " ip_limit; read -rp " Quota GB: " gb_limit
               ...
               echo -e "${CYAN}GET / HTTP/1.1[crlf]Host: ${cur_dom}[crlf]Upgrade: websocket[crlf][crlf]${NC}"
               press_any_key ;;
...
uninstall_panel() {
    clear
    echo -e "${ALERT_RED}⚠️ WARNING: PURGING ENTIRE SYSTEM PANEL PLATFORM ⚠️${NC}"
    ...
    if [ "$un_confirm" == "y" ]; then
        ...
        rm -rf /etc/nginx/conf.d/vpn.conf /etc/xray /etc/slowdns /etc/zainuxbrand /usr/local/bin/autokill.py /usr/local/bin/ws-proxy.py
        rm -f /etc/systemd/system/autokill.service /etc/systemd/system/ws-proxy.service /etc/systemd/system/tgbot.service
