...
uninstall_panel() {
    ...
    if [ "$un_confirm" == "y" ]; then
        ...
        rm -rf /etc/nginx/conf.d/vpn.conf /etc/xray /etc/slowdns /etc/zainuxbrand /usr/local/bin/autokill.py /usr/local/bin/ws-proxy.py /usr/local/bin/xray /usr/local/bin/dnstt-server
